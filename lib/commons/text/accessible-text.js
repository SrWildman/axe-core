...
function shouldCheckSubtree(element) {
	return ['BUTTON', 'SUMMARY', 'A'].includes(element.nodeName.toUpperCase());
}

function shouldNeverCheckSubtree(element) {
	return ['TABLE', 'FIGURE'].includes(element.nodeName.toUpperCase());
}
...
function formValueText(element) {
	...
	if (nodeName === 'INPUT') {
		if (!element.hasAttribute('type') || (
				inputTypes.includes(element.getAttribute('type').toLowerCase())) &&
				element.value) {
...
function isEmbeddedControl(e) {
	...
	switch (e.nodeName.toUpperCase()) {
		...
		case 'TEXTAREA':
			return true;
		case 'INPUT':
			return !e.hasAttribute('type') || (inputTypes.includes(e.getAttribute('type').toLowerCase()));
		default:
			return false;
	}
}
		...
		if (isInput(element) && !inControlContext) {
			if (isButton(element)) {
				return (
					...
					defaultButtonValues[(element.getAttribute('type') || '').toLowerCase()] ||
					''
				);
			}
